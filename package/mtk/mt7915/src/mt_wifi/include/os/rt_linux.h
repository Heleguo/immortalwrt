...
#define AP_DRIVER_VERSION			"7.4.0.0"
...
#define STA_DRIVER_VERSION		"7.4.0.0"
...
#else
#define  mtk_printk(fmt, ...) printk(KERN_CONT fmt, ##__VA_ARGS__)
...
#define ASSERT(x)                                                               \
	{                                                                               \
		if (!(x)) {                                                                   \
			mtk_printk(KERN_WARNING __FILE__ ":%d assert " #x "failed\n", __LINE__);    \
			dump_stack();\
