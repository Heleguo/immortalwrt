...
PNDIS_PACKET ClonePacket(BOOLEAN MonitorOn, PNET_DEV ndev, PNDIS_PACKET pkt, UCHAR *buf, ULONG sz)
{
	...
	if (pkt == NULL) {
		MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Error! pkt is NULL\n", __func__));
		ASSERT(pkt);
		return NULL;
	}
...
void hex_dump_with_cat_and_lvl(char *str, UCHAR *pSrcBufVA, UINT SrcBufLen, INT dbgcat, INT dbg_sub_cat, INT dbglvl)
{
	...
	for (x = 0; x < SrcBufLen; x++) {
		...
		MTWF_LOG(dbgcat, dbg_sub_cat, dbglvl, ("%02x ", ((unsigned char)pSrcBufVA[x])));
		...
		for (j = 0; j < len; j++) {
			...
			if (skb->head) {
				...
				printk("%s(): index:%d, page:%p ,rfcnt:%d, page size:%zu, order:%u, dtor:%u\n", __func__, k++, p,
					OS_PAGE_REF(p), page_size, (unsigned int) compound_order(p));
			}
			skb = skb->next;
		}
